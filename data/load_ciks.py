...
def create_dfs(overwrite=False):
    ...
    if ~path.exists(cikfilename) or overwrite:
        ...
        master_ciks = master_ciks.reindex(
            columns=master_ciks.columns.tolist() + cik_cols
        )
...
def check_ncen():
    ...
    latest_ncens = master_ncens.loc[
        master_ncens.reset_index().groupby("CIK")["fileDate"].idxmax()
    ]
    ...
    master_ciks = master_ciks.merge(
        latest_ncens, how="left", on="CIK Number", suffixes=("", "_y")
    )
    ind1 = (master_ciks["ncen_date"] != master_ciks["ncen_date"]) & (
        master_ciks["ncen_date_y"] == master_ciks["ncen_date_y"]
    )
    ind2 = master_ciks["ncen_date_y"] > master_ciks["ncen_date"]
    replace_ind = ind1 | ind2
    master_ciks.loc[replace_ind, "ncen_date"] = master_ciks.loc[
        replace_ind, "ncen_date_y"
    ]
    master_ciks.loc[replace_ind, "ncen_url"] = master_ciks.loc[
        replace_ind, "ncen_url_y"
    ]
    ...
    for index, row in master_ciks.iterrows():
        if row["new_ncen"]:
            ...
            if url == url:
                ...
                master_ciks.loc[index, ["infamily", "fundfamily", "new_ncen"]] = (
                    infam,
                    fam,
                    False,
                )
...
def check_lvl3(check_date=datetime(2020, 12, 31), date_delta=3, check="nan"):
    ...
    ind = (master_urls["fileDate"] > check_date) & (
        master_urls["fileDate"] <= (check_date + relativedelta(months=date_delta))
    )
        ...
        if proceed_check:
            ...
            if len(nport_urls) > 0:
                ...
                for url in nport_urls:
                    lvl3 = (
                        lvl3
                        or s.get(url).text.find("<fairValLevel>3</fairValLevel>") > 0
                    )
                    time.sleep(0.2)
                master_ciks.loc[
                    index, "lvl3"
                ] = lvl3  # has lvl3 = true, does not have lvl3 = false, no nport = nan
                numcik += 1
...
def map_fundManager(tmpoverride=True):
    ...
    master_ciks.loc[~famind, "fundManager_raw"] = master_ciks.loc[
        ~famind, "Entity Name"
    ]
...
#     print('Looped through', numcik, 'CIKs in', toc-tic, 'secs. Number of nan entries remaining for NCEN data: ',
#           num_nan_end)
