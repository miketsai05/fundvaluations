...
def gen_table(unicorn_name, selected_units):
    cols = [
        ...
        "title",
        "filingURL",
    ]  # don't need this in both functions
    ...
    tmptable["Fund_xml"] = (
        "[" + tmptable["seriesname"].astype(str) + "](" + tmptable["filingURL"].str[:-4]
    )
    tmptable["Fund_xml"] = (
        tmptable["Fund_xml"].str.replace("-", "")
        + "/xslFormNPORT-P_X01/primary_doc.xml)"
    )
    tmptable["Fund_html"] = (
        "["
        + tmptable["seriesname"].astype(str)
        + "]("
        + tmptable["filingURL"].astype(str)
        + ")"
    )
...
def gen_table_format():
    ...
    dt_cols = [
        {"name": x, "id": y, "type": z, "presentation": a}
        for x, y, z, a in zip(colnames, cols, coltype, colpresentation)
    ]
...
unicornsfilename = "data/unicorn_summary.pkl"
unicorn_summary = pd.read_pickle(unicornsfilename)
unicornset = unicorn_summary["unicorn"]
        ...
        html.Div(
            dt.DataTable(
                ...
                style_data_conditional=[
                    {"if": {"column_id": "Fund_xml"}, "padding-top": 15}
                ],
                sort_action="native",
                sort_mode="multi",
                sort_by=[
                    {"column_id": "valDate", "direction": "desc"},
                    {"column_id": "fundManager", "direction": "asc"},
                ],
            )
        ),
    ]
...
def update_graph(input_value, selected_units):
    ...
    lbunit = ["Equity", "Debt", "Derivatives", "Other"]
    vunit = ["NS", "PA", "NC", "OU"]
    dunit = [x not in available_units for x in vunit]
    unitoptions = [
        {"label": lb, "value": v, "disabled": d}
        for lb, v, d in zip(lbunit, vunit, dunit)
    ]

    manageroptions = [
        {"label": i, "value": i}
        for i in sorted(tmptable["fundManager"].astype(str).unique())
    ]
    managerval = sorted(tmptable["fundManager"].astype(str).unique())

    dateoptions = [
        {"label": i, "value": i}
        for i in sorted(tmptable["valDate"].unique(), reverse=True)
    ]
...
def filter_table(selectmanager, selectdate, data):
    ...
    tmptable = tmptable[
        tmptable["fundManager"].isin(selectmanager)
        & tmptable["valDate"].isin(selectdate)
    ]
