...
def gen_summary_data():
    ...
    tmptable1["unicornlink"] = (
        "["
        + tmptable1["unicorn"].astype(str)
        + "](/apps/app_company#"
        + tmptable1["unicorn"].str.replace(" ", "_").astype(str)
        + ")"
    )
...
def gen_table_format():
    ...
    colheader = [
        "",
        "",
        "Most Recent Quarter Valuations (Per Share)",
        "Most Recent Quarter Valuations (Per Share)",
        "Most Recent Quarter Valuations (Per Share)",
        "Most Recent Quarter Valuations (Per Share)",
        "Most Recent Quarter Valuations (Per Share)",
        "Most Recent Quarter Valuations (Per Share)",
        "Valuation Dates Available",
        "Valuation Dates Available",
    ]
    colnames = [
        "Company",
    ...
    dt_cols = [
        {"name": [w, x], "id": y, "type": z, "presentation": a}
        for w, x, y, z, a in zip(colheader, colnames, cols, coltype, colpresentation)
    ]
        ...
        dbc.Container(
            dt.DataTable(
                ...
                style_data_conditional=[
                    {"if": {"column_id": "unicornlink"}, "padding-top": 15}
                ],
                tooltip_data=[
                    {"fundManagerunique": x}
                    for x in gen_summary_data()["fundManagerunique"]
                ],
